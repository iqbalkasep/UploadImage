...
import com.iqbal.myapplication.URIPathHelper
import okhttp3.*
...
//    var serverUploadDirectoryPath: String = "https://handyopinion.com/tutorials/uploads/"
    var serverURL: String = "http://34.101.86.104/upload"
    var serverUploadDirectoryPath: String = "http://34.101.86.104/upload"
    ...
    fun uploadFile(sourceFile: File, uploadedFileName: String? = null) {
        Thread {
            Log.d("Source File", "Source File "+sourceFile)
            Log.d("uploadedFileName", "uploadedFileName" + uploadedFileName)
//            val mimeType = getMimeType(sourceFile);
            val mimeType = MimeTypeMap.getFileExtensionFromUrl(sourceFile.toString());
//            val extension = MimeTypeMap.getFileExtensionFromUrl(url)
            if (mimeType == null) {
                Log.e("file error Yes", "Not able to get mime type Yes")
                return@Thread
            }
            ...
            try {
                ...
                Log.d("File Response","Response, "+response)
                if (response.isSuccessful) {
                    Log.d("File upload","success, path: $serverUploadDirectoryPath$fileName")
                    showToast("File uploaded successfully at $serverUploadDirectoryPath$fileName")
                } else {
                    Log.e("File upload", "failed Yes")
                    showToast("File uploading failed")
                }
            } catch (ex: Exception) {
                ex.printStackTrace()
                Log.e("File upload", "failed exception catch")
                showToast("File uploading failed")
            }
